/**
 * CreditSniper — Verified Download Endpoint
 *
 * Verifies Stripe checkout session, then streams product zip from R2.
 * CF Pages Function: creditsniper.xyz/api/download?session_id=xxx&product=slug
 *
 * Environment bindings required:
 *   STRIPE_SECRET_KEY  - Stripe secret key (env var)
 *   CREDIT_VAULT       - R2 bucket binding (creditsniper-vault)
 */

// Product slug → R2 zip filename mapping
const PRODUCT_MAP = {
  "credit-sample-pack":    "Credit_Sample_Pack.zip",
  "dispute-letter-pack":   "Dispute_Letter_Pack.zip",
  "full-credit-vault":     "Full_Credit_Vault.zip",
  "pro-monthly-1k":        "Full_Credit_Vault.zip",
  "enterprise-annual":     "Full_Credit_Vault.zip",
};
...
async function handleRequest(context) {
  ...
  // Fetch from R2
  const object = await env.CREDIT_VAULT.get(r2Key);
